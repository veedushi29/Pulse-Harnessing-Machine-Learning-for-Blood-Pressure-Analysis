...
# ── Stage metadata (-1 = Hypotension, was missing in original) ──
STAGES = {
...
# ── Complete ACC/AHA rule-based classification ───────────────
...
# ── Clinical alert engine (entirely new) ────────────────────
...
def predict():
    try:
        ...
        # FIX: Correct MAP formula = dia + PP/3 (original used (sys+2*dia)/3 which is equivalent but less clear)
        ...
        # ML model — only run when BP is not hypotensive (model has no hypotension class)
        if model is not None and rule_stage >= 0:
            try:
                ...
                # ML outputs 0-3; map to our extended 0-4 scale
